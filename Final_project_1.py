...
user_search = st.sidebar.text_input("1. Type a movie title keyword:", value="Toy Story")
...
if not matching_movies.empty:
    ...
    with col_m1:
        st.metric(
            label="Approach A (Similarity Mix) Category Accuracy",
            value=f"{mean_accuracy_A * 100:.1f}%",
            delta=f"{(mean_accuracy_A - mean_accuracy_B) * 100:.1f}% vs Approach B"
        )
    with col_m2:
        st.metric(
            label="Approach B (Strict Genre) Category Accuracy",
