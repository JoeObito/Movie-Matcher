...
if not matching_movies.empty:
    ...
    with col_m1:
        st.metric(
            ...
            delta=f"{(mean_accuracy_A - mean_accuracy_B) * 100:.1f}% vs Strict Genre"
        )
    with col_m2:
        st.metric(
            label="Strict Genre Category Accuracy",
            value=f"{mean_accuracy_B * 100:.1f}%",
            delta=f"{(mean_accuracy_B - mean_accuracy_A) * 100:.1f}% vs Similarity Mix"
        )
